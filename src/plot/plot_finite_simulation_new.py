...
import matplotlib
matplotlib.use("Agg")  # backend non-interattivo (evita Qt/Wayland)
...
def metric_to_label(metric: str, *, delta: bool) -> str:
    ...
    if metric.endswith("util"):        return "Δ Utilizzazione (‰)" if delta else "Utilizzazione"
...
def _fixed_decimals_formatter(digits: int):
    return FuncFormatter(lambda x, pos: f"{x:.{digits}f}")

# -------------------- PLOT: spaghetti generico (no legend) -------------------- #
def spaghetti_no_legend(df: pd.DataFrame,
    ...
    plt.xlabel(xlabel)
    plt.ylabel(metric_to_label(metric, delta=False))
...
# -------------------- PLOT: utilizzazione (assoluta/delta) -------------------- #
def _util_axis(ax, ymin_obs: float, ymax_obs: float, *, delta: bool, y_digits: int):
    head = 1e-5 if not delta else 1e-3
    lo = (max(0.0, ymin_obs - head) if not delta else ymin_obs - head)
    hi = ymax_obs + head
    ax.set_ylim(lo, hi)
    ax.yaxis.set_major_locator(MaxNLocator(6))
    ax.yaxis.set_major_formatter(_fixed_decimals_formatter(y_digits))
    ax.axhline(0.0 if delta else 1.0, color='gray', linewidth=0.8, alpha=0.6)
...
def spaghetti_util(df: pd.DataFrame,
    ...
    ymin_obs, ymax_obs = float("inf"), float("-inf")
    for _, g in d.groupby(REP_COL):
        gx = g[TIME_COL].to_numpy() * scale
        gy_raw = g[metric].to_numpy()
        gy = (gy_raw - 1.0) * 1e3 if delta else gy_raw
        ymin_obs = min(ymin_obs, float(gy.min()))
        ymax_obs = max(ymax_obs, float(gy.max()))
        plt.plot(gx, gy, linewidth=1.2)

    if vline_sec is not None:
        plt.axvline(x=vline_sec * scale, linestyle="--")

    _util_axis(ax, ymin_obs, ymax_obs, delta=delta, y_digits=y_digits)

    plt.xlabel(xlabel)
    plt.ylabel(metric_to_label(metric, delta=delta))
    plt.title(f"{metric} — orizzonte finito" + (" (Δ‰)" if delta else ""))
    plt.grid(True)
    plt.tight_layout()

    suffix = "_delta_permille" if delta else ""
    outpath = outdir / f"{metric}_spaghetti{suffix}.png"
    plt.savefig(outpath, dpi=150, bbox_inches="tight")
    plt.close()
    return outpath
...
def spaghetti_util_zoom(df: pd.DataFrame,
    ...
    vals = ((d[metric] - 1.0) * 1e3) if delta else d[metric]
    ymin_obs = float(vals.min())
    ymax_obs = float(vals.max())

    spread = max(ymax_obs - ymin_obs, 1e-9)
    pad = max(1e-3 if delta else 3e-4, spread * 0.30)
    low, high = ymin_obs - pad, ymax_obs + pad

    plt.figure()
    ax = plt.gca()
    for _, g in d.groupby(REP_COL):
        gx = g[TIME_COL].to_numpy() * scale
        gy = ((g[metric].to_numpy() - 1.0) * 1e3) if delta else g[metric].to_numpy()
        plt.plot(gx, gy, linewidth=1.2)
    ...
    plt.xlabel(xlabel)
    plt.ylabel(metric_to_label(metric, delta=delta))
    plt.title(f"{metric} — orizzonte finito (zoom)" + (" (Δ‰)" if delta else ""))
    plt.grid(True)
    plt.tight_layout()

    suffix = "_delta_permille" if delta else ""
    outpath = outdir / f"{metric}_spaghetti_zoom{suffix}.png"
    plt.savefig(outpath, dpi=150, bbox_inches="tight")
    plt.close()
    return outpath
...
# -------------------- MAIN -------------------- #
def main():
    ap = argparse.ArgumentParser(description="Grafici orizzonte finito + QoS")
    ...
    ap.add_argument("--time-unit", type=str, default="h",
                    choices=["s","h","hour","day"], help="Unità asse X")
    ...
    ap.add_argument("--delta", action="store_true",
                    help="Per metriche *_util, plotta Δ(util-1) in ‰")
    ap.add_argument("--y-digits", type=int, default=6,
                    help="Decimali mostrati sull'asse Y")
    args = ap.parse_args()
    ...
    metrics = list(dict.fromkeys(metrics_base + metrics_spike))
    ...
    for metric in metrics:
        if metric.endswith("util"):
            spaghetti_util(
                df, metric, scale, xlabel, outdir,
                round_time_decimals=round_time_decimals, vline_sec=args.vline,
                delta=args.delta, y_digits=args.y_digits
            )
            spaghetti_util_zoom(
                df, metric, scale, xlabel, outdir,
                round_time_decimals=round_time_decimals, vline_sec=args.vline,
